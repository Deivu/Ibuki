...
use crate::models::{ApiTrack, ApiTrackResult, ApiTrackInfo};
...
impl Spotify {
    ...
    async fn get_token(&self) -> Option<String> {
        ...
        if let Some(config) = &CONFIG.spotify_config {
            if let (Some(client_id), Some(client_secret)) = (&config.client_id, &config.client_secret) {
                ...
                let res = self.http.post("https://accounts.spotify.com/api/token")
                ...
                if res.status().is_success() {
                    ...
                    if let Some(access_token) = json["access_token"].as_str() {
                        ...
                        *expiry = SystemTime::now() + std::time::Duration::from_secs(expires_in - 60);
    ...
    fn parse_track(&self, item: &Value) -> Option<ApiTrack> {
        ...
        let authors = item["artists"].as_array()?
            .iter()
            .filter_map(|a| a["name"].as_str())
            .collect::<Vec<_>>()
            .join(", ");
        let length = item["duration_ms"].as_u64()?;
        let artwork_url = item["album"]["images"].as_array()
            .and_then(|imgs| imgs.first())
            .and_then(|img| img["url"].as_str())
            .map(|s| s.to_string());
        let isrc = item["external_ids"]["isrc"].as_str().map(|s| s.to_string());
        let uri = item["external_urls"]["spotify"].as_str().unwrap_or("").to_string();
    ...
    async fn get_track(&self, id: &str, token: &str) -> Option<ApiTrack> {
        let res = self.http.get(format!("https://api.spotify.com/v1/tracks/{}", id))
            .header("Authorization", format!("Bearer {}", token))
            .send()
            .await
            .ok()?;
    ...
    async fn get_album_tracks(&self, id: &str, token: &str) -> Option<Vec<ApiTrack>> {
         let res = self.http.get(format!("https://api.spotify.com/v1/albums/{}/tracks", id))
            .header("Authorization", format!("Bearer {}", token))
             .query(&[("limit", "50")])
            .send()
            .await
        ...
        let album_res = self.http.get(format!("https://api.spotify.com/v1/albums/{}", id))
             .header("Authorization", format!("Bearer {}", token))
             .send()
             .await
             .ok()?;
        ...
        let artwork_url = album_json["images"].as_array()
            .and_then(|imgs| imgs.first())
            .and_then(|img| img["url"].as_str())
            .map(|s| s.to_string());
    ...
    async fn get_playlist_tracks(&self, id: &str, token: &str) -> Option<Vec<ApiTrack>> {
        let res = self.http.get(format!("https://api.spotify.com/v1/playlists/{}/tracks", id))
            .header("Authorization", format!("Bearer {}", token))
            .query(&[("limit", "50")])
            .query(&[("market", CONFIG.spotify_config.as_ref().and_then(|c| c.market.clone()).as_deref().unwrap_or("US"))])
            .send()
            .await
            .ok()?;
...
impl Source for Spotify {
    ...
    async fn init(&self) -> Result<(), ResolverError> {
        if let Some(token) = self.get_token().await {
            tracing::info!("Spotify Source Initialized with token: {}...", &token[0..10]);
        } else {
    ...
    async fn resolve(&self, query: Query) -> Result<Option<ApiTrackResult>, ResolverError> {
        ...
        match query {
            ...
            Query::Url(url) => {                
                ...
                if let Some(type_idx) = parts.iter().position(|&x| x == "track" || x == "album" || x == "playlist") {
                             ...
                             "album" => {
                                 if let Some(tracks) = self.get_album_tracks(id, &token).await {
                                     ...
                                     let name = format!("Spotify Album {}", id); // Should fetch actual name
                                      return Ok(Some(ApiTrackResult::Playlist(crate::models::ApiTrackPlaylist {
    ...
    async fn make_playable(&self, _track: ApiTrack) -> Result<Input, ResolverError> {
        ...
        Err(ResolverError::Custom("Playback failed: Could not find track on YouTube".to_string()))
    }
}
