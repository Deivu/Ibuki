...
impl Source for Songlink {
    ...
    fn parse_query(&self, query: &str) -> Option<Query> {
        if query.contains("song.link")
            || query.contains("album.link")
            || query.contains("artist.link")
            || query.contains("odesli.co")
        {
    ...
    async fn make_playable(&self, _track: ApiTrack) -> Result<Input, ResolverError> {
        Err(ResolverError::Custom(
            "Songlink source does not support direct playback".to_string(),
        ))
    }
}
