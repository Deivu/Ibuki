...
impl YoutubeOAuth {
    ...
    async fn refresh_access_token(&self, refresh_token: &str) -> Result<OauthToken, ResolverError> {
        let params = [
            ("client_id", "861556708454-d6dlm3lh05ig8aa4ea9398830989024p.apps.googleusercontent.com"), // Common YouTube TV client ID
        ...
        if !res.status().is_success() {
             ...
             return Err(ResolverError::Custom(format!("OAuth Refresh Failed: {}", text)));
        }

        let body: Value = res.json().await.map_err(ResolverError::Reqwest)?;
        
        let access_token = body.get("access_token").and_then(|v| v.as_str()).ok_or(ResolverError::Custom("No access_token".to_string()))?.to_string();
        let expires_in = body.get("expires_in").and_then(|v| v.as_u64()).unwrap_or(3600);
        let new_refresh_token = body.get("refresh_token").and_then(|v| v.as_str()).map(|s| s.to_string()).unwrap_or(refresh_token.to_string());
