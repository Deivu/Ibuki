...
pub async fn fetch_visitor_data(http_client: &Client) -> Option<String> {
   ...
   if !response.status().is_success() {
       tracing::warn!(
           "Failed to fetch visitor data, status: {}",
           response.status()
       );
       return None;
   }
   ...
   if let Some(captures) = visitor_regex.captures(&body) {
       let visitor_data = captures.get(1)?.as_str().to_string();
       tracing::debug!("Fetched visitor data from embed page");
       return Some(visitor_data);
   }
   
   tracing::debug!("Visitor data not found in embed page, trying guide API");

   let guide_body = serde_json::json!({
       "context": {
           "client": {
   ...
   let guide_response = http_client
       .post(format!("{}/youtubei/v1/guide", YOUTUBE_API_URL))
       .json(&guide_body)
       .send()
       .await
       .ok()?;

   if !guide_response.status().is_success() {
       return None;
   }

   let guide_data: serde_json::Value = guide_response.json().await.ok()?;
   ...
   guide_data
       ...
       .map(|s| {
           tracing::debug!("Fetched visitor data from guide API");
           s.to_string()
       })
}
